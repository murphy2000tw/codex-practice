...
實機測試發現，新增日文單字中頁後，單字練習頁仍保留舊版「單字功能」方塊。該方塊位於單字練習 view 內，標題為「單字功能」，並包含「單字練習」與「單字測驗」兩個切換按鈕，因此和新的單字中頁分流入口重複。

### 2. 「單字功能」方塊原本用途

「單字功能」方塊原本用於同一個單字頁內切換單字卡練習與單字測驗：點擊「單字練習」會切回卡片模式，點擊「單字測驗」會進入測驗模式。這是在尚未拆出單字中頁前的主要練習 / 測驗切換入口。

### 3. 為什麼新增單字中頁後它變成重複入口

目前正式流程已改為「日文學習 → 單字 → 單字練習 / 單字測驗」。單字中頁已經是唯一負責選擇練習或測驗的頁面，因此單字練習頁內再次顯示「單字練習 / 單字測驗」切換，會造成重複入口，也讓使用者在練習頁內又能跳到測驗頁。

### 4. 本次移除內容

本次只移除單字練習頁內的 vocabulary mode panel，包括「單字功能」標題、說明文字、「單字練習」按鈕與「單字測驗」按鈕。分類篩選、詞性分類、程度篩選、搜尋單字、單字卡控制與單字卡列表均保留。

### 5. 單字練習頁目前結構

單字練習頁現在保留「返回單字選單」、分類篩選、詞性分類、程度篩選、搜尋單字、單字卡控制與單字卡列表。不再顯示「單字功能」方塊，也不再顯示單字練習 / 單字測驗切換按鈕。

### 6. 單字測驗頁目前結構

單字測驗頁仍維持乾淨測驗 view，只顯示返回單字選單、測驗標題、題目、選項、作答回饋、下一題、重新開始測驗與統計資訊。測驗頁仍不顯示分類篩選、程度篩選、搜尋單字、單字功能或單字卡列表。

### 7. 單字中頁是否未受影響

單字中頁未修改，仍保留返回日文首頁、「單字練習」與「單字測驗」兩個入口。單字測驗仍可從單字中頁進入，單字練習頁與測驗頁的返回單字選單也仍回到中頁。

### 8. 文法 / 閱讀是否未受影響

本次未修改文法頁、閱讀頁、英文頁面、閱讀題庫或 ruby 規則。文法與閱讀仍由既有日文主入口流程控制。

### 9. 防呆檢查結果

已更新 `scripts/check-japanese-vocabulary-view-split.js` 與 `scripts/check-japanese-vocabulary-menu.js`，明確檢查單字中頁保留「單字練習 / 單字測驗」，單字練習頁不得包含「單字功能」、`cardModeButton`、`quizModeButton` 或 `data-mode-panel="vocab"`，同時仍需包含分類篩選、程度篩選、搜尋單字與單字卡列表。`scripts/check-japanese-view-isolation.js` 也同步更新為不再把「單字功能」視為單字 view 必要內容。

### 10. 三個主要 script 結果

本次回歸需通過 `node scripts/auditSiteHealth.js`、`node scripts/audit-reading-vocabulary.js` 與 `node scripts/check-reading-ruby.js`。另需執行 `node scripts/check-japanese-main-entry.js`、`node scripts/check-japanese-view-isolation.js`、`node scripts/check-japanese-vocabulary-view-split.js` 與 `node scripts/check-japanese-vocabulary-menu.js` 確認日文主入口、單字中頁、單字練習頁與測驗頁隔離狀態。

### 11. `vocabulary.json` 筆數確認

本次未修改 `vocabulary.json`。需以 Python 檢查確認正式單字庫仍為 3179 筆。

### 12. 是否仍有殘留問題

本次目標範圍內未發現殘留問題。單字練習頁已移除重複功能方塊；單字練習 / 單字測驗切換只保留在單字中頁。

## 後續修正：閱讀中頁與練習 / 測驗入口分離

### 本次目的

本次修正將「閱讀」從原本進入後直接展開練習或測驗內容，改成先進入獨立閱讀中頁。目的在於讓日文首頁的「進入閱讀」只負責進入閱讀選單，使用者再明確選擇「閱讀練習」或「閱讀測驗」，避免文章、題目或舊模式切換按鈕直接混在閱讀入口頁。
...
閱讀中頁保留「返回日文首頁」、主標題「日文閱讀」與說明「選擇要進行閱讀練習或閱讀測驗」。中頁只顯示兩張大卡片：「閱讀練習」與「閱讀測驗」。閱讀中頁不直接顯示閱讀文章、閱讀題目、測驗題目、測驗選項，也不顯示舊 tab 式閱讀切換列或多餘管理按鈕。

### 閱讀練習頁結構

點擊「閱讀練習」後，程式切換到 `practice` 閱讀 view，隱藏閱讀中頁卡片，顯示「返回閱讀選單」與既有閱讀練習內容。閱讀練習仍顯示文章、題目、選項與作答回饋，並保留 ruby / 假名輔助顯示。

### 閱讀測驗頁結構

點擊「閱讀測驗」後，程式切換到 `quiz` 閱讀 view，隱藏閱讀中頁卡片，顯示「返回閱讀選單」與既有閱讀測驗內容。閱讀測驗仍顯示測驗進度、題目、選項、下一篇閱讀 / 查看結果 / 重新開始測驗等必要流程，且維持不顯示 ruby / 假名。

### 返回閱讀選單邏輯

閱讀練習與閱讀測驗內容現在都透過共用的內容 render helper 插入「返回閱讀選單」按鈕。點擊後會將閱讀狀態切回 `menu`，清空並隱藏閱讀內容容器，再顯示閱讀中頁的「閱讀練習 / 閱讀測驗」兩張卡片。閱讀中頁本身仍保留「返回日文首頁」，可回到日文首頁四大方塊。

### Ruby 與測驗顯示規則

閱讀練習仍使用既有 `showRuby: true` 流程，因此文章、題目與選項保留 ruby / 假名輔助。閱讀測驗仍使用既有 `showRuby: false` 流程，因此測驗文章、題目與選項不顯示 ruby / 假名。本次未修改閱讀文章、題庫、ruby 規則或閱讀資料來源。

### 單字 / 文法影響範圍

本次只調整閱讀中頁與閱讀練習 / 測驗入口分離。單字中頁、單字練習、單字測驗乾淨頁、文法入口與日文首頁四大方塊的既有流程未被改成新資料或新功能。本次未修改 `vocabulary.json`，未新增日文單字，未新增閱讀文章，也未進行 Batch 07。

### 防呆檢查結果

新增 `scripts/check-japanese-reading-menu.js`，檢查閱讀 view 具備 `menu / practice / quiz` 三段狀態、閱讀首頁預設進入中頁、中頁只保留閱讀練習與閱讀測驗入口、中頁不直接渲染文章 / 題目 / 選項、練習與測驗內容都具備返回閱讀選單，以及練習維持 `showRuby: true`、測驗維持 `showRuby: false`。
...
已執行 Python 檢查，`vocabulary.json` 仍為 3179 筆。本次未修改 `vocabulary.json`。

### 是否仍有殘留問題

靜態檢查未發現閱讀中頁仍直接顯示文章、題目、測驗選項或舊 tab 的殘留問題。此環境未提供已安裝的瀏覽器 smoke test 條件；若需要視覺驗收，建議後續以實機或 Playwright 補做桌機與手機 viewport 截圖。
