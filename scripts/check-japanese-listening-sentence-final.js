...
function requireExcludes(source, snippet, label) {
  if (source.includes(snippet)) fail(`Unexpected ${label}: ${snippet}`);
}
...
let questions = [];
if (!arraySource) {
  fail('JAPANESE_LISTENING_QUESTIONS constant does not exist.');
} else {
  try {
    questions = vm.runInNewContext(arraySource);
  } catch (error) {
    fail(`JAPANESE_LISTENING_QUESTIONS could not be parsed: ${error.message}`);
  }
}

if (questions.length !== 10) fail(`JAPANESE_LISTENING_QUESTIONS should contain exactly 10 questions, found ${questions.length}.`);
...
questions.forEach((question, index) => {
  ...
  if (question.options[question.answerIndex] !== question.zh) fail(`Question ${index + 1} correct option does not match zh.`);
});
...
requireIncludes(script, 'function resetJapaneseListeningState', 'resetJapaneseListeningState function');
requireIncludes(script, 'function renderJapaneseListeningView', 'renderJapaneseListeningView function');
requireIncludes(resultRenderer, 'const total = JAPANESE_LISTENING_QUESTIONS.length', 'quiz result total uses question length');
...
requireIncludes(practiceRenderer, '(currentListeningIndex + 1) % JAPANESE_LISTENING_QUESTIONS.length', 'practice cycles through questions');
...
requireIncludes(quizRenderer, 'JAPANESE_LISTENING_QUESTIONS.length', 'quiz progress total');
