...
function checkUiAndBaselines() {
  ...
  assert([...html.matchAll(/data-japanese-entry="([^"]+)"/g)].map((m) => m[1]).join(',') === 'vocabulary,reading,grammar,listening,reviewMenu,jlptMock', 'home entries must remain original five plus JLPT mock last');
  ...
  assert(!/熟練度|弱點排行|間隔複習|統計圖表|每日提醒|雲端同步|帳號系統|匯出|匯入/i.test(html + style + script), 'forbidden future feature text/code found');
