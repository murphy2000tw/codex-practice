...
assert(run('git merge-base --is-ancestor 7000eb35b8d26650c8e50a953a7a6bddd8e95be3 HEAD; echo $?') === '0', 'HEAD must contain PR #276 merge commit 7000eb35b8d26650c8e50a953a7a6bddd8e95be3');
let baseQuestions = [];
...
let dupIds=0, dupSentences=0, missing=0, invalidGrammar=0, invalidChunkTextType=0, emptyChunks=0, punctuationOnlyChunks=0, chunkWithoutWordChar=0, mapping=0, reconstruction=0, forbiddenAmbiguityAdmission=0;
...
const forbiddenAmbiguityAdmissions = [/也自然/, /可成立/, /可用/, /也常見/, /不是錯/, /唯一性弱/, /可移動但本題固定/, /本題指定第一空/, /語法可通/, /雖可/, /雖常見/];
...
for (const q of questions) {
  ...
  if (ambiguityFixIds.has(q.id)) assertAmbiguityFixShape(q);
  if (expectedNew.has(q.id)) { newStars[q.starSlot]++; if (pos >= 0) newPos[pos]++; assert(q.uniqueAnswerReviewed === true, `${q.id}: uniqueAnswerReviewed must be true`); assert(/已檢查24種排列/.test(q.reviewNote), `${q.id}: reviewNote must document 24 permutations`); const forbiddenHit = forbiddenAmbiguityAdmissions.find(re => re.test(q.reviewNote || '')); if (forbiddenHit) { forbiddenAmbiguityAdmission++; failures.push(`${q.id}: reviewNote contains forbidden ambiguity admission ${forbiddenHit}`); } assert(q.completeSentence === expectedSentences.get(q.id), `${q.id}: locked completeSentence changed`); }
}
