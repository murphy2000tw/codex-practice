...
const batch13bOrthographicAliasRules = [
  { surface: 'パン＿＿', module: 'grammar', ids: ['n5-grammar-009', 'n5-grammar-235'], lemma: 'パン', vocabularyId: 132, evidence: 'cloze underline removed' },
  { surface: 'クラス＿＿', module: 'grammar', ids: ['n5-grammar-214'], lemma: 'クラス', vocabularyId: 3189, evidence: 'cloze underline removed' },
  { surface: 'けしゴム', module: 'reading', ids: ['jp-reading-set-n4-031'], lemma: '消しゴム', vocabularyId: 3226, evidence: 'kana/kanji mixed orthography' },
];
const batch13bGrammarElementRules = [
  { surface: '何時', module: 'grammar', ids: ['n5-grammar-224', 'n5-grammar-225', 'n5-grammar-226'], evidence: '何時 / 何時 + に／ですか' },
  { surface: '何時', module: 'reading', ids: ['jp-reading-q-n4-022-01', 'jp-reading-q-n4-027-01', 'jp-reading-q-n4-067-01', 'jp-reading-q-n4-070-01', 'jp-reading-q-n4-097-03', 'jp-reading-q-n4-101-01', 'jp-reading-q-n4-102-02'], evidence: 'Batch 13B report lists reading as 何時 source module' },
  { surface: '何時', module: 'listening', ids: ['jl-024'], evidence: 'Batch 13B report lists listening as 何時 source module' },
  { surface: 'ごろ', module: 'grammar', ids: ['n5-grammar-228', 'n5-grammar-229'], evidence: '〜ごろ / 時間點 + ごろ' },
  { surface: 'のに', module: 'grammar', ids: ['n4-grammar-108'], evidence: '〜のに / 普通形 + のに' },
  { surface: '何月何日', module: 'grammar', ids: ['n5-grammar-226'], evidence: '何月何日 / 何月何日 + ですか' },
  { surface: 'ずに', module: 'grammar', ids: ['n4-grammar-246'], evidence: '〜ずに / 動詞ない形去い + ずに' },
  { surface: 'た形', module: 'grammar', ids: ['n5-grammar-033', 'n5-grammar-039'], evidence: 'ました / て形 grammar structure' },
  { surface: 'やら', module: 'grammar', ids: ['n4-grammar-177'], evidence: '〜やら〜やら / 名詞 + やら + 名詞 + やら' },
  { surface: 'いた', module: 'grammar', ids: ['n4-grammar-203'], evidence: '〜と聞く / 聞いた' },
  { surface: 'えて', module: 'grammar', ids: ['n4-grammar-180'], evidence: '〜に加えて / 名詞 + に加えて' },
  { surface: 'える', module: 'grammar', ids: ['n4-grammar-165'], evidence: '〜ように見える / 普通形 + ように見える' },
  { surface: 'おく', module: 'grammar', ids: ['n4-grammar-053'], evidence: '〜ておく / 動詞て形 + おく' },
  { surface: 'きゃ', module: 'grammar', ids: ['n4-grammar-138'], evidence: '〜なきゃ / 動詞ない形去い + きゃ' },
  { surface: 'くて', module: 'grammar', ids: ['n4-grammar-087'], evidence: '〜ても / い形容詞くて + も' },
  { surface: 'けど', module: 'grammar', ids: ['n4-grammar-197'], evidence: '〜けど / 普通形 + けど' },
  { surface: 'ける', module: 'grammar', ids: ['n4-grammar-057'], evidence: '〜続ける / 動詞ます形去掉ます + 続ける' },
  { surface: 'こう', module: 'grammar', ids: ['n5-grammar-230'], evidence: 'ここ／そこ／あそこ grammar source' },
  { surface: 'ずつ', module: 'grammar', ids: ['n4-grammar-247'], evidence: '〜ないで / 動詞ない形 + で' },
  { surface: 'って', module: 'grammar', ids: ['n4-grammar-199'], evidence: '〜たって / 動詞た形 + って' },
  { surface: 'てき', module: 'grammar', ids: ['n4-grammar-243'], evidence: '〜てくる / 動詞て形 + くる' },
  { surface: 'てみ', module: 'grammar', ids: ['n4-grammar-243'], evidence: '〜てくる / 動詞て形 + くる' },
  { surface: 'でも', module: 'grammar', ids: ['n5-grammar-046'], evidence: 'が（但是） / 句子 + が + 句子' },
  { surface: 'など', module: 'grammar', ids: ['n4-grammar-100'], evidence: '〜や〜など / 名詞 + や + 名詞 + など' },
  { surface: 'のが', module: 'grammar', ids: ['n4-grammar-150'], evidence: '〜のが得意だ／苦手だ' },
  { surface: 'のは', module: 'grammar', ids: ['n4-grammar-210'], evidence: '〜のは〜だ / 動詞普通形 + のは' },
  { surface: 'はず', module: 'grammar', ids: ['n4-grammar-248'], evidence: '〜つもりだ / 動詞辞書形／ない形 + つもりだ' },
  { surface: 'まま', module: 'grammar', ids: ['n4-grammar-208'], evidence: '〜まま / 動詞た形／ない形 + まま' },
  { surface: 'める', module: 'grammar', ids: ['n4-grammar-051'], evidence: '〜始める / 動詞ます形去掉ます + 始める' },
  { surface: 'やる', module: 'grammar', ids: ['n4-grammar-096'], evidence: '〜てやる / 動詞て形 + やる' },
  { surface: '何月', module: 'grammar', ids: ['n5-grammar-225'], evidence: '何曜日 / 何曜日 + ですか' },
  { surface: '何枚', module: 'grammar', ids: ['n5-grammar-224'], evidence: '何時 / 何時 + に／ですか' },
];
...
function findBatch13bOrthographicAlias(src, token) {
  const rule = batch13bOrthographicAliasRules.find((r) => r.surface === token && r.module === src.module && r.ids.includes(src.id));
  if (!rule) return null;
  const vocab = vocabulary.find((v) => v.id === rule.vocabularyId && v.word === rule.lemma);
  if (!vocab) return null;
  if (token.replace(/＿/g, '') !== rule.lemma && rule.surface !== 'けしゴム') return null;
  if (rule.surface === 'けしゴム' && vocab.kana !== 'けしごむ') return null;
  return { status: 'orthographic-alias-match', lemma: rule.lemma, type: `Batch 13B source-bound alias: vocabulary ${rule.vocabularyId}; ${rule.evidence}` };
}
function findBatch13bGrammarElement(src, token) {
  const rule = batch13bGrammarElementRules.find((r) => r.surface === token && r.module === src.module && r.ids.includes(src.id));
  if (!rule) return null;
  const item = src.module === 'grammar' ? grammar.find((g) => g.id === src.id) : null;
  if (src.module === 'grammar' && !item) return null;
  if (src.module === 'grammar' && !rule.evidence) return null;
  return { status: 'grammar-element-source-bound', lemma: token, type: `Batch 13B grammar source-bound: ${rule.evidence}` };
}
...
function processToken(src, token, explicit = false) {
  ...
  if (norms.length && norms[0].status !== 'grammar-element') return add(src, token, norms[0], explicit);
  const alias = findBatch13bOrthographicAlias(src, token);
  if (alias) return add(src, token, alias, explicit);
  const grammarElement = findBatch13bGrammarElement(src, token);
  if (grammarElement) return add(src, token, grammarElement, explicit);
...
const orthographicAliasAll = uniq('orthographic-alias-match').sort((a, b) => a.lemma.localeCompare(b.lemma, 'ja'));
const grammarSourceBoundAll = uniq('grammar-element-source-bound').sort((a, b) => a.lemma.localeCompare(b.lemma, 'ja'));
