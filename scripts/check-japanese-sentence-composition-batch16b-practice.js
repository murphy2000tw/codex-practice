...
assert(/句子重組/.test(grammarMenu) && /排列四個片段，完成正確句子/.test(grammarMenu) && /開始句子重組/.test(grammarMenu), 'entry title/description/action mismatch');
...
assert(/question\.before/.test(script) && /question\.after/.test(script) && /question\.chunks/.test(script), 'before/chunks/after rendering missing');
assert(/currentSentenceCompositionAnswer\.push/.test(script) && /removeSentenceCompositionChunk/.test(script), 'select and return chunk flow missing');
assert(/清除重排/.test(script) && /確認答案/.test(script) && /下一題/.test(script), 'required action buttons missing');
assert(/currentSentenceCompositionAnswer\.length < 4/.test(script) && /請先填滿四格再確認/.test(script) && !/alert\(/.test(sentenceFns), 'incomplete answer status without alert missing');
assert(/currentSentenceCompositionLocked = true/.test(script) && /disabled = selected \|\| currentSentenceCompositionLocked/.test(script), 'locking after submit missing');
assert(/correctOrder\.join/.test(script), 'answer judgment must use correctOrder');
assert(/答對了/.test(script) && /再看看正確順序/.test(script), 'correct/wrong feedback missing');
assert(/completeSentence/.test(script) && /kana/.test(script) && /meaning/.test(script) && /explanation/.test(script), 'complete sentence/kana/meaning/explanation feedback missing');
...
assert(!/starSlot[\s\S]{0,120}(===|==|correct|answer|判分)/.test(script), 'practice must not judge by starSlot');
...
assert(/sentence-composition/.test(css) && /overflow-wrap: anywhere/.test(css) && /env\(safe-area-inset-bottom/.test(css) && /:focus-visible/.test(css) && /@media \(max-width: 640px\)/.test(css), 'mobile/focus/safe-area CSS missing');
