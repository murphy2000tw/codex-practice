...
## N4 動詞・形容詞・副詞加強批次紀錄（第 2 批）

- 新增範圍：`vocabulary.json` id 2501–2800，共新增 300 筆 JLPT N4 常見單字。
- 詞性分配：
  - 動詞：100 筆
  - い形容詞：80 筆
  - な形容詞：60 筆
  - 副詞：60 筆
- 新增後 `vocabulary.json` 總筆數：2800 筆。
- 本次新增資料的 `level` 全部為 `N4`，且未修改既有 N5 或既有 N4 單字內容。
- 本次未修改 `index.html`、`style.css`、`script.js`。
...
- `vocabulary.json` 已確認為合法 JSON。
- 總筆數由 2500 筆增加為 2800 筆，正確增加 300 筆。
- `id` 已確認由 1 到 2800 連續且不重複。
- `word` 已確認全檔不重複。
- 所有資料皆包含 `id`、`level`、`word`、`kana`、`meaning`、`partOfSpeech`、`example`、`exampleKana`、`exampleMeaning`。
- 已確認必要欄位沒有空白值。
- 已確認所有 `exampleKana` 不含漢字。
- 已確認本次新增 300 筆的 `exampleMeaning` 不含日文假名或片假名，避免中文釋義混入日文助詞或奇怪混合句。
- 已確認本次新增 300 筆詞性統計如下：動詞 100 筆、い形容詞 80 筆、な形容詞 60 筆、副詞 60 筆。

### 仍需人工確認項目

- 本批新增詞彙與例句已通過結構、欄位、重複、假名與詞性數量檢查；目前未列出需人工確認的阻塞項目。
- 若後續要進一步提高教材品質，建議由日語母語者或 JLPT 教材編輯再抽樣確認各單字是否完全符合目標 N4 難度範圍。

## 測驗模式新增紀錄

- 新增日期：2026-06-04。
- 本次未修改 `vocabulary.json`，也未新增、刪除或調整任何單字資料。
- 本次修改 `index.html`、`style.css`、`script.js`，新增「單字卡模式／測驗模式」切換區；預設維持原本的單字卡模式。
- 測驗模式會依照目前的程度篩選、詞性分類與搜尋結果出題，只顯示一道日文單字與假名，並提供四個隨機排序的中文意思選項。
- 每題包含 1 個正確答案與 3 個從同一批篩選結果中隨機抽出的錯誤選項；答題後會顯示答對或答錯提示，並顯示「下一題」按鈕。
- 新增分數統計：已答題數、答對題數、正確率，並提供「重新開始測驗」按鈕將分數歸零並重新出題。
- 若目前篩選結果不足 4 個單字，測驗模式會顯示提示：「目前篩選結果不足 4 個單字，請調整分類或搜尋條件。」

### 檢查結果

- 已使用 `node --check script.js` 確認 JavaScript 語法正確。
- 已以 Node DOM stub 自動流程檢查單字卡模式仍可顯示單字、顯示答案、換一組單字。
- 已以 Node DOM stub 自動流程檢查測驗模式可正常出題，四個選項可點選，答對／答錯提示會出現。
- 已以 Node DOM stub 自動流程檢查答題後會顯示「下一題」，按下後會產生新題目。
- 已以 Node DOM stub 自動流程檢查分數統計會隨答題更新，且「重新開始測驗」會將統計歸零。
- 已以 Node DOM stub 自動流程檢查搜尋結果少於 4 個單字時會顯示不足提示。

## N4 全詞性補強批次紀錄（第 3 批）

- 新增日期：2026-06-04。
- 本次新增總數：300 筆。
- 新增範圍：`vocabulary.json` id 2801 至 3100。
- 所有本批新增單字的 `level` 均為 `N4`。
...
- 新增前 `vocabulary.json` 總筆數：2800 筆。
- 新增後 `vocabulary.json` 總筆數：3100 筆。

### 檢查結果摘要

- 已確認 `vocabulary.json` 仍為合法 JSON。
- 已確認總筆數由 2800 筆增加為 3100 筆，正好增加 300 筆。
- 已確認 id 由 1 至 3100 連續排列，沒有重複或缺號。
- 已確認所有 `word` 在整份 `vocabulary.json` 中沒有重複。
- 已確認每筆資料均包含 `id`、`level`、`word`、`kana`、`meaning`、`partOfSpeech`、`example`、`exampleKana`、`exampleMeaning`。
- 已確認沒有空白欄位。
- 已確認所有 `exampleKana` 不含漢字。
- 已確認所有 `exampleMeaning` 不含日文假名混入。
- 已確認本批新增 300 筆的詞性分配符合指定數量。

### 仍需人工確認項目

- 本批單字已依常見生活、學校、工作、交通、狀態與語氣表達挑選；仍建議由日語母語者或 JLPT 教材編輯抽樣確認所有詞彙是否完全符合目標 N4 難度與教材風格。
