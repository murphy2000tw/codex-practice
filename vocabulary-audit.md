...
- 總筆數：1000。
- 必要欄位：`id`、`word`、`kana`、`meaning`、`partOfSpeech`、`example`、`exampleKana`、`exampleMeaning`。
- 本報告僅記錄發現的可疑或需修正項目；未直接修改 `vocabulary.json`。
...
- `word` 重複：未發現。
- `id` 重複：未發現。
- `id` 不連續：未發現，id 由 1 到 1000 連續。
- `meaning` 混入日文假名：未發現明顯案例。
- 主要問題集中在 id 662–921 的數量詞例句：多筆 `exampleKana` 仍含漢字，且 `exampleMeaning` 混入日文假名、日文漢字或日文助詞。
- 另有部分例句雖有使用到 `word` 或同讀音 `kana`，但語義搭配不自然，已列於下表。
