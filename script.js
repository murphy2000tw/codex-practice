...
const startJapaneseJlptMockButton = document.querySelector("#startJapaneseJlptMock");
const japaneseJlptQuestionContent = document.querySelector("#japaneseJlptQuestionContent");
const japaneseJlptStartActions = document.querySelector("#japaneseJlptStartActions");
const japaneseJlptUnavailableNote = document.querySelector("#japanese-jlpt-unavailable-note");
...
let japaneseJlptQuestionBank = null;
...
async function switchJapaneseTab(tab) {
  ...
  if (normalizedTab === "vocabulary") {
    ...
    await loadJapaneseJlptProductBanks();
  }
}
...
const JAPANESE_JLPT_PRODUCT_PROFILE_VERSION = "17c10-product-v1";
const JAPANESE_JLPT_PRODUCT_PROFILE_ID = "site-jlpt-style-product";
...
function validateJapaneseJlptQuestionBank(data) {
  if (
    ...
    data.policyVersion !== JAPANESE_JLPT_POLICY_VERSION ||
  ...
  JAPANESE_JLPT_LEVELS.forEach((level) => {
    ...
    const vocabularyMeaning = questions.filter(
      (question) =>
        question.section === "vocabulary" &&
        question.questionType === "meaning",
    );
    const grammarMeaning = questions.filter(
      (question) =>
        question.section === "grammar" && question.questionType === "meaning",
    );
    const grammarCloze = questions.filter(
      (question) =>
        question.section === "grammar" && question.questionType === "cloze",
    );
    if (
      questions.length !== 20 ||
    ...
    questions.forEach((question) => {
      if (
        question.kanjiPolicy !== "kana-replacement" ||
...
function validateJapaneseJlptReadingBank(data) {
  ...
  const profile =
    ...
    data.selectionProfiles.N4 &&
    data.selectionProfiles.N4.initial;
  if (
    !data ||
    data.schemaVersion !== 1 ||
    data.dataVersion !== JAPANESE_JLPT_READING_DATA_VERSION ||
    data.policyVersion !== JAPANESE_JLPT_READING_POLICY_VERSION
  )
    throw new Error("閱讀題庫格式或版本不符");
  if (
    !availability ||
    ...
    throw new Error("閱讀題庫可用狀態或統計不符");
  if (
    !profile ||
    profile.profileVersion !== JAPANESE_JLPT_READING_PROFILE_ID ||
    profile.selectionMode !== "fixed-manifest" ||
    profile.setCount !== 10 ||
    profile.questionCount !== 14 ||
    !Array.isArray(profile.setIds) ||
    profile.setIds.length !== 10 ||
    profile.setIds.some(
      (id, index) => id !== JAPANESE_JLPT_READING_SET_IDS[index],
    ) ||
    new Set(profile.setIds).size !== 10
  )
    throw new Error("固定閱讀 manifest 不符");
  if (!Array.isArray(data.readingSets) || data.readingSets.length !== 105)
    throw new Error("閱讀資料組格式不符");
  const selectedSets = profile.setIds.map((id) =>
    data.readingSets.find((set) => set.id === id),
  );
  if (selectedSets.some((set) => !set))
    throw new Error("固定閱讀 manifest 找不到資料組");
  let questionCount = 0;
  selectedSets.forEach((set) => {
    if (
      set.level !== "N4" ||
      !isNonEmptyString(set.id) ||
      !isNonEmptyString(set.sourceSetId) ||
      !isNonEmptyString(set.displayTitle) ||
      !isNonEmptyString(set.displayPassage) ||
      !isNonEmptyString(set.passageKana) ||
      !Array.isArray(set.rubyTerms) ||
      !set.rubyCoverage ||
      typeof set.rubyCoverage !== "object" ||
      !isNonEmptyString(set.rubyCoverage.status) ||
      !Array.isArray(set.rubyCoverage.coveredTerms) ||
      !Array.isArray(set.rubyCoverage.uncoveredHan) ||
      !Array.isArray(set.questions) ||
      set.questions.length === 0
    )
      throw new Error(`${set.id || "未知資料組"} 閱讀本文或 ruby 資料不完整`);
    set.rubyTerms.forEach((term) => {
      if (
        !term ||
        !isNonEmptyString(term.text) ||
        !isNonEmptyString(term.reading)
      )
    ...
    set.questions.forEach((question, questionIndex) => {
      if (
        ...
        question.answerIndex < 0 ||
        question.answerIndex > 3 ||
        !isNonEmptyString(question.answerDisplay) ||
        !isNonEmptyString(question.explanation)
      )
  ...
  if (selectedSets.length !== 10 || questionCount !== 14)
...
const JAPANESE_JLPT_N5_READING_SET_METADATA_CONTRACT = deepFreezeJapaneseJlptValue({
  "jlpt-reading-17c9b-n5-short-passage-bakery-breakfast": ["みじかいぶん", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-short-passage-rainy-park-plan": ["みじかいぶん", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-medium-passage-family-seaside": ["ちゅうくらいのぶん", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-medium-passage-morning-train": ["ちゅうくらいのぶん", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-information-search-fruit-shop-prices": ["ねだんひょう", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-information-search-library-hours": ["じかんひょう", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-notice-and-message-home-curry-message": ["いえのメッセージ", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
  "jlpt-reading-17c9b-n5-notice-and-message-school-trip-notice": ["がっこうのおしらせ", "表示文はひらがな・カタカナ中心とし、漢字を使わないため ruby の推測は不要。"],
});
const JAPANESE_JLPT_N4_TYPE_TO_SECTION_CONTRACT_VERSION = "17c2-type-to-section-v1";
const JAPANESE_JLPT_N4_TYPE_TO_SECTION_CONTRACT = deepFreezeJapaneseJlptValue({
...
const JAPANESE_JLPT_N5_GENERIC_DISTRACTOR_REASON = /^(?:(?:這個)?(?:選項|答案)?(?:是)?(?:不正確|不符合題意|錯誤|不對|不是正確答案)|這個選項不符合題目要求，因此不是正確答案|本文と一致しないため不正解|這個答案與本文不同，所以不能選)[。.]?$/u;
...
function validateJapaneseJlptN5InformationMaterial(set, question) {
  ...
  if (!uniqueIds(material.headings) || !uniqueIds(material.columns) || !uniqueIds(material.rows) ||
      material.headings.some((heading) => !isNonEmptyString(heading.label)) ||
      material.columns.some((column) => !isNonEmptyString(column.label)))
    throw new Error("N5 information material identity 無效");
  if (material.rows.some((row) => !Array.isArray(row.cells) || row.cells.length !== material.columns.length))
    throw new Error("N5 information row width 無效");
  const cells = material.rows.flatMap((row) => row.cells);
  const allMaterialIds = [...material.headings, ...material.columns, ...material.rows, ...cells].map((item) => item.id);
  if (!uniqueIds(cells) || cells.some((cell) => !isNonEmptyString(cell.text)) ||
      new Set(allMaterialIds).size !== allMaterialIds.length)
  ...
  question.informationEvidence.forEach((evidence) => {
    ...
    if (!row || evidenceRows.has(evidence.rowId) || !Array.isArray(evidence.cellIds) || evidence.cellIds.length < 2)
...
function validateJapaneseJlptReadingSet(set, expectedLevel, sourceBank) {
  ...
  const auditedMetadata = expectedLevel === "N5" && JAPANESE_JLPT_N5_READING_SET_METADATA_CONTRACT[set && set.id];
  ...
  if (expectedLevel === "N5" && (!audited || !auditedMetadata || set.sourceSetId !== set.id || set.section !== audited[0] ||
      set.type !== auditedMetadata[0] || set.sourceType !== auditedMetadata[0] || set.type !== set.sourceType ||
      set.kanjiPolicy !== auditedMetadata[1] || set.rubyCoverage.status !== "complete" ||
      set.rubyCoverage.coveredTerms.length !== 0 || set.rubyCoverage.uncoveredHan.length !== 0 ||
      set.rubyTerms.length !== 0 ||
  ...
  set.rubyTerms.forEach((term) => {
    ...
    if (expectedLevel === "N5" && /[一-龯々〆ヵヶ]/u.test(term.reading))
      throw new Error("N5 ruby reading 不得包含漢字");
  });
  set.questions.forEach((question, questionIndex) => {
    ...
    if (expectedLevel === "N5") {
      ...
      const normalizedReasons = rejected.map((review) => review.reason.trim().replace(/[。.!！?？]+$/u, ""));
      if (supported.length !== 1 || supported[0].optionIndex !== question.answerIndex || rejected.length !== 3 ||
          new Set(normalizedReasons).size !== 3 || rejected.some((review) =>
            review.reason.trim().length < 4 || JAPANESE_JLPT_N5_GENERIC_DISTRACTOR_REASON.test(review.reason.trim())))
        throw new Error("N5 option review verdict contract 無效");
      ...
      (question.passageEvidence || []).forEach((evidence) => {
        ...
        if (!evidence || !isNonEmptyString(evidence.spanId) || spanIds.has(evidence.spanId) ||
            ...
            evidence.endCodePoint > points.length ||
...
function createJapaneseJlptReadingCandidates(data, level) {
  ...
  const expectedN5Sets = { "short-passage": 2, "medium-passage": 2, "information-search": 2, "notice-and-message": 2 };
  if (!data || data.schemaVersion !== (n5 ? "1.0.0" : 1) ||
      ...
      (n5 ? data.derivationVersion !== "17c9c-v1" || data.sourceVersion !== "17c9b-v1" ||
        ...
        !data.inventory.sections || JAPANESE_JLPT_READING_SECTIONS.some((section) => {
          const inventory = data.inventory.sections[section];
          return !inventory || inventory.setCount !== expectedN5Sets[section] ||
            inventory.questionCount !== expected[section];
        }) || Object.keys(data.inventory.sections).length !== JAPANESE_JLPT_READING_SECTIONS.length
        : data.policyVersion !== "17c2-reading-internal-v1" || !data.availability || !data.availability.N5 ||
          ...
          data.availability.N4.questionCount !== 150 ||
          JSON.stringify(data.typeToSection) !== JSON.stringify(JAPANESE_JLPT_N4_TYPE_TO_SECTION_CONTRACT)) ||
      !Array.isArray(data.readingSets) ||
  ...
  data.readingSets.forEach((set, setIndex) => {
    ...
    if (!n5) {
      ...
      const allowedTypes = JAPANESE_JLPT_N4_TYPE_TO_SECTION_CONTRACT[set.section];
  ...
  JAPANESE_JLPT_READING_SECTIONS.forEach((section) => {
    ...
    if (n5 && (data.readingSets.filter((set) => set.section === section).length !== expectedN5Sets[section] ||
        data.readingSets.filter((set) => set.section === section)
          .reduce((count, set) => count + set.questions.length, 0) !== expected[section]))
      throw new Error(`N5/${section} actual inventory 無效`);
  });
  return candidates;
}
...
const JAPANESE_JLPT_PRODUCT_BANKS = Object.freeze([
  ["vocabulary auto", "JAPANESE_JLPT_VOCABULARY_AUTO_URL", "japaneseJlptVocabularyAutoQuestions.json"],
  ["vocabulary semantic", "JAPANESE_JLPT_VOCABULARY_SEMANTIC_URL", "japaneseJlptVocabularySemanticQuestions.json"],
  ["grammar form-selection", "JAPANESE_JLPT_GRAMMAR_FORM_SELECTION_URL", "japaneseJlptGrammarFormSelectionQuestions.json"],
  ["sentence-composition", "JAPANESE_JLPT_SENTENCE_COMPOSITION_URL", "japaneseSentenceCompositionQuestions.json"],
  ["N5 reading", "JAPANESE_JLPT_READING_N5_URL", "japaneseJlptReadingN5Questions.json"],
  ["N4 reading", "JAPANESE_JLPT_READING_N4_URL", "japaneseJlptReadingQuestions.json"],
]);

async function buildJapaneseJlptProductCandidates(fetchProvider = fetch) {
  if (typeof fetchProvider !== "function") throw new TypeError("JLPT fetch provider 無效");
  const responses = await Promise.all(JAPANESE_JLPT_PRODUCT_BANKS.map(async ([label, globalName, path]) => {
    const response = await fetchProvider(window[globalName] || path);
    if (!response || !response.ok) throw new Error(`${label}: HTTP ${response && response.status || "未知"}`);
    return [label, response];
  }));
  const parsed = await Promise.all(responses.map(async ([label, response]) => {
    try { return await response.json(); }
    catch (error) { throw new Error(`${label}: JSON 解析失敗（${error instanceof Error ? error.message : "未知錯誤"}）`); }
  }));
  const [auto, semantic, formSelection, sentenceComposition, n5Reading, n4Reading] = parsed;
  const candidates = [
    ...createJapaneseJlptVocabularyDerivedCandidates(auto, semantic),
    ...createJapaneseJlptGrammarFormSelectionCandidates(formSelection),
  ...
  for (const level of JAPANESE_JLPT_LEVELS) {
    const { profile, levelProfile } = validateJapaneseJlptProfile(
      JAPANESE_JLPT_PROFILE_REGISTRY, JAPANESE_JLPT_PRODUCT_PROFILE_VERSION,
      JAPANESE_JLPT_PRODUCT_PROFILE_ID, level,
    );
    prepareJapaneseJlptCandidatePools(level, profile, levelProfile, candidates, profile.profileVersion);
  }
  return candidates;
}

async function loadJapaneseJlptProductBanks(fetchProvider = fetch) {
  if (japaneseJlptProductCandidates || japaneseJlptIsLoading) return;
  japaneseJlptIsLoading = true;
  japaneseJlptProductLoadError = "";
  renderJapaneseJlptPanel();
  try {
...
async function loadJapaneseJlptQuestionBank(fetchProvider = fetch) {
  ...
  try {
    const response = await fetchProvider(
      window.JAPANESE_JLPT_VOCABULARY_GRAMMAR_URL ||
        "japaneseJlptVocabularyGrammarQuestions.json?v=17b1",
    );
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    japaneseJlptQuestionBank = validateJapaneseJlptQuestionBank(
      await response.json(),
    );
  } catch (error) {
...
async function loadJapaneseJlptReadingBank(fetchProvider = fetch) {
  if (japaneseJlptReadingBank || japaneseJlptReadingIsLoading) return;
  japaneseJlptReadingIsLoading = true;
  japaneseJlptReadingLoadError = "";
  renderJapaneseJlptPanel();
  try {
    const response = await fetchProvider(
      window.JAPANESE_JLPT_READING_URL ||
        "japaneseJlptReadingQuestions.json?v=17c2",
    );
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    japaneseJlptReadingBank = validateJapaneseJlptReadingBank(
      await response.json(),
    );
  } catch (error) {
    ...
    japaneseJlptReadingIsLoading = false;
    renderJapaneseJlptPanel();
  }
}
...
function renderJapaneseJlptPanel() {
  japaneseJlptLevelButtons.forEach((button) => {
    const selected =
      button.dataset.japaneseJlptLevel === selectedJapaneseJlptLevel;
  ...
  if (selectedJapaneseJlptLevel)
    selectedLevelProfile = validateJapaneseJlptProfile(
      JAPANESE_JLPT_PROFILE_REGISTRY, japaneseJlptActiveProfileVersion,
      japaneseJlptActiveProfileId, selectedJapaneseJlptLevel,
    ).levelProfile;
  const productActive = japaneseJlptActiveProfileVersion === JAPANESE_JLPT_PRODUCT_PROFILE_VERSION;
  const canStart = Boolean(
    selectedLevelProfile && !japaneseJlptSession &&
    (productActive ? japaneseJlptProductCandidates : japaneseJlptQuestionBank) &&
    (productActive || !selectedLevelProfile.sections.reading.included || japaneseJlptReadingBank),
  );
  if (startJapaneseJlptMockButton)
    startJapaneseJlptMockButton.disabled = !canStart;
  if (japaneseJlptIsLoading || japaneseJlptReadingIsLoading) {
    const p = document.createElement("p");
    p.textContent = "正在獨立載入並驗證 JLPT 題庫…";
    japaneseJlptStatus.appendChild(p);
  }
  if (japaneseJlptLoadError) {
    const p = document.createElement("p");
    p.textContent = japaneseJlptLoadError;
    japaneseJlptStatus.appendChild(p);
    return;
  }
  if (japaneseJlptProductLoadError) {
    const p = document.createElement("p");
    p.textContent = japaneseJlptProductLoadError;
    japaneseJlptStatus.appendChild(p);
  }
  if (!productActive && !japaneseJlptQuestionBank) {
    const p = document.createElement("p");
    p.textContent = "JLPT 單字・文法題庫尚未載入。";
    japaneseJlptStatus.appendChild(p);
    return;
  }
  if (!selectedJapaneseJlptLevel) {
    const p = document.createElement("p");
    p.textContent = "請選擇 N5 或 N4 查看目前可用資料狀態。";
    japaneseJlptStatus.appendChild(p);
    return;
  }
  ...
  heading.textContent = `${productActive ? "正式模式" : "相容模式"}・已選擇 ${selectedJapaneseJlptLevel}・固定共 ${levelProfile.total} 題`;
  ...
  reading.textContent = productActive
    ? `閱讀 ${levelProfile.sections.reading.total} 題`
    : selectedJapaneseJlptLevel === "N5"
      ? "閱讀：相容模式未啟用"
      : japaneseJlptReadingLoadError ||
...
function selectJapaneseJlptLevel(level) {
  if (JAPANESE_JLPT_LEVELS.includes(level) && !japaneseJlptSession) {
    selectedJapaneseJlptLevel = level;
    renderJapaneseJlptPanel();
  }
}
function getJapaneseJlptRandomIndex(maxExclusive) {
  if (!Number.isSafeInteger(maxExclusive) || maxExclusive <= 0)
    throw new RangeError("JLPT 隨機索引範圍無效");
...
function shuffleJapaneseJlptArray(items, randomIndexProvider = getJapaneseJlptRandomIndex) {
  if (!Array.isArray(items) || typeof randomIndexProvider !== "function")
    throw new TypeError("JLPT 洗牌參數無效");
  const shuffled = [...items];
  for (let index = shuffled.length - 1; index > 0; index -= 1) {
    const swapIndex = randomIndexProvider(index + 1);
    if (!Number.isInteger(swapIndex) || swapIndex < 0 || swapIndex > index)
      throw new RangeError("JLPT 隨機索引供應器回傳無效值");
...
function createBalancedJapaneseJlptAnswerPositions(questionCount, randomIndexProvider = getJapaneseJlptRandomIndex) {
  if (!Number.isSafeInteger(questionCount) || questionCount <= 0)
    throw new RangeError("JLPT 題數無效");
  const positionPriority = shuffleJapaneseJlptArray(
    [0, 1, 2, 3],
    randomIndexProvider,
  );
  const counts = [0, 1, 2, 3].map(
    (position) =>
      Math.floor(questionCount / 4) +
      (positionPriority.indexOf(position) < questionCount % 4 ? 1 : 0),
  );
  const positions = counts.flatMap((count, position) =>
    Array.from({ length: count }, () => position),
  );
  return shuffleJapaneseJlptArray(positions, randomIndexProvider);
}
function randomizeJapaneseJlptQuestionOptions(questionSnapshot, targetAnswerIndex, randomIndexProvider = getJapaneseJlptRandomIndex) {
  if (
    ...
    questionSnapshot.answerIndex < 0 ||
    questionSnapshot.answerIndex > 3 ||
    !Number.isInteger(targetAnswerIndex) ||
    targetAnswerIndex < 0 ||
    targetAnswerIndex > 3
  )
  ...
  if (shuffledAnswerIndex < 0) throw new Error("JLPT 正確選項無法解析");
  [shuffledOptions[targetAnswerIndex], shuffledOptions[shuffledAnswerIndex]] = [
...
function createJapaneseJlptReadingSnapshots(selectedSets) {
  return selectedSets.flatMap((set, setIndex) =>
    set.questions.map((question, questionIndex) => ({
      ...
      rubyTerms: set.rubyTerms.map((term) => ({
        text: term.text,
        reading: term.reading,
      })),
      rubyCoverage: {
        status: set.rubyCoverage.status,
        coveredTerms: [...set.rubyCoverage.coveredTerms],
        uncoveredHan: [...set.rubyCoverage.uncoveredHan],
      },
      displayText: question.displayText,
      options: [...question.options],
      answerIndex: question.answerIndex,
    ...
    for (const [questionType, quota] of Object.entries(section.questionTypes)) {
      ...
      normalizedCandidates.forEach((candidate) => {
        ...
        if (identities.has(identity))
          throw new Error(`JLPT candidate canonical identity 重複：${identity}`);
        identities.add(identity); uniquePool.push(candidate);
      });
...
function appendJapaneseJlptRubyText(parent, value, rubyTerms) {
  renderRubyParts(parent, createRubyPartsFromTerms(value, rubyTerms));
}
function appendJapaneseJlptLabeledTable(parent, material, rubyTerms) {
  if (!material || material.type !== "labeled-table" || !Array.isArray(material.columns) ||
      !Array.isArray(material.rows)) return false;
  const wrapper = document.createElement("div");
  wrapper.className = "jlpt-reading-table-wrap";
  (Array.isArray(material.headings) ? material.headings : []).forEach((item) => {
    if (!item || !isNonEmptyString(item.label)) return;
    const heading = document.createElement("h5");
    appendJapaneseJlptRubyText(heading, item.label, rubyTerms);
  ...
  material.columns.forEach((column) => {
    ...
    appendJapaneseJlptRubyText(th, column && isNonEmptyString(column.label) ? column.label : "—", rubyTerms);
  ...
  material.rows.forEach((row) => {
    ...
    material.columns.forEach((_, index) => {
      const td = document.createElement("td");
      const cell = cells[index];
      appendJapaneseJlptRubyText(td, cell && isNonEmptyString(cell.text) ? cell.text : "—", rubyTerms);
      tr.appendChild(td);
    });
...
function appendJapaneseJlptNewTypeAnswerDetails(parent, question) {
  ...
  handler();
  if (!isNonEmptyString(question.answerDisplay))
    detail("正確答案", question.options && question.options[question.answerIndex]);
  detail("解析", question.explanation);
  return true;
}
function appendJapaneseJlptAnswerFeedbackDetails(parent, question) {
  if (appendJapaneseJlptNewTypeAnswerDetails(parent, question)) return;
  const labels = question.section === "vocabulary"
    ? { word: "單字", kana: "假名", meaning: "意思" }
    : { grammar: "文法", kana: "假名", answer: "答案", meaning: "意思", structure: "結構" };
  if (question.answerDisplay && typeof question.answerDisplay === "object" && !Array.isArray(question.answerDisplay)) {
    Object.entries(question.answerDisplay).forEach(([key, value]) => {
      if (typeof value === "string") appendJapaneseJlptDetail(parent, labels[key] || key, value);
    });
  } else {
    const fallback = isNonEmptyString(question.answerDisplay)
      ? question.answerDisplay : question.options && question.options[question.answerIndex];
...
function answerJapaneseJlptQuestion(selectedIndex) {
  ...
  const question =
    japaneseJlptSession.questionSnapshots[japaneseJlptSession.currentIndex];
  if (japaneseJlptSession.answers[japaneseJlptSession.currentIndex]) return;
  japaneseJlptSession.answers[japaneseJlptSession.currentIndex] = {
    selectedIndex,
...
function renderJapaneseJlptQuestion() {
  ...
  const { questionSnapshots, currentIndex, answers, selectedLevel } =
    japaneseJlptSession;
  ...
  if (isReading) {
    ...
    readingProgress.className = "quiz-prompt-label";
    readingProgress.textContent = `閱讀資料 ${question.readingSetIndex + 1}／${question.readingSetCount}・問題 ${question.readingQuestionIndex + 1}／${question.readingQuestionCount}`;
    const title = document.createElement("h4");
    appendJapaneseJlptRubyText(
      title,
      question.displayTitle,
      question.rubyTerms,
    );
    card.append(readingProgress, title);
    if (!appendJapaneseJlptLabeledTable(card, question.material, question.rubyTerms)) {
      const passage = document.createElement("p");
      passage.className = "reading-passage";
      appendJapaneseJlptRubyText(passage, question.displayPassage, question.rubyTerms);
  ...
  const questionTypeLabels = {
    "kanji-reading": "漢字讀音", orthography: "漢字寫法", context: "語境選擇",
    paraphrase: "近義改寫", usage: "用法判斷", "form-selection": "文法選形",
    "sentence-composition": "句子重組", "short-passage": "短篇閱讀",
  ...
  question.options.forEach((option, index) => {
    ...
    if (isReading)
      appendJapaneseJlptRubyText(button, option, question.rubyTerms);
    else button.textContent = option;
    button.disabled = Boolean(answer);
    if (answer && index === question.answerIndex)
      button.classList.add("is-correct");
    if (answer && index === answer.selectedIndex && !answer.isCorrect)
  ...
  next.textContent =
    currentIndex === questionSnapshots.length - 1 ? "完成本輪作答" : "下一題";
  next.disabled = !answer;
  next.addEventListener("click", advanceJapaneseJlptQuestion);
  card.append(type, prompt, options, feedback, next);
...
function renderJapaneseJlptCompletion() {
  ...
  japaneseJlptQuestionContent.append(heading, note, back);
  heading.focus();
}
...
function startJapaneseJlptMock() {
  const productActive = japaneseJlptActiveProfileVersion === JAPANESE_JLPT_PRODUCT_PROFILE_VERSION;
  if (!selectedJapaneseJlptLevel || (productActive ? !japaneseJlptProductCandidates : !japaneseJlptQuestionBank)) return;
  const { levelProfile } = validateJapaneseJlptProfile(
    JAPANESE_JLPT_PROFILE_REGISTRY, japaneseJlptActiveProfileVersion,
    japaneseJlptActiveProfileId, selectedJapaneseJlptLevel,
  );
  if (
    !productActive && levelProfile.sections.reading.included && !japaneseJlptReadingBank
  )
    return;
  let nextSession;
  try {
    ...
    if (japaneseJlptStatus) {
      ...
      message.textContent = `JLPT session 建立失敗：${error instanceof Error ? error.message : "未知錯誤"}。請返回設定後再試一次。`;
      japaneseJlptStatus.appendChild(message);
    }
    return;
  }
...
function normalizeJapaneseVocabularyView(view) {
  ...
  if (["menu", "practice", "quiz", "sentence-composition-practice", "sentence-composition-quiz"].includes(view)) return view;
  return "menu";
}
...
function renderJapaneseGrammarView(view = japaneseGrammarView) {
  ...
  const isSentenceCompositionView = japaneseGrammarView === "sentence-composition-practice" || japaneseGrammarView === "sentence-composition-quiz";
...
function resetJapaneseSentenceCompositionPracticeState() {
  currentSentenceCompositionQuestion = null;
  currentSentenceCompositionAnswer = [];
  currentSentenceCompositionLocked = false;
}

function resetJapaneseSentenceCompositionQuizSetupState() {
  selectedSentenceCompositionQuizLevel = null;
...
function renderJapaneseSentenceCompositionPractice(statusText = "") {
  ...
  wrapper.append(title, mode, meta, filters, questionLine, prompt, bank, status, actions);
...
function selectSentenceCompositionChunk(chunkId) {
  ...
  renderJapaneseSentenceCompositionPractice("已選擇選項，可改選或確認。");
}
...
function getSentenceCompositionQuestionCounts() {
  return {
    all: sentenceCompositionQuestions.length,
    N5: sentenceCompositionQuestions.filter((q) => q.level === "N5").length,
    N4: sentenceCompositionQuestions.filter((q) => q.level === "N4").length,
  };
}

function renderJapaneseSentenceCompositionQuizSetup() {
  if (!sentenceCompositionContent) return;
  if (!sentenceCompositionHasLoaded) {
    sentenceCompositionContent.textContent = "正在載入句子重組題庫資訊…";
    ensureSentenceCompositionLoaded().then((loaded) => { if (loaded) renderJapaneseSentenceCompositionQuizSetup(); });
    return;
  }
  const counts = getSentenceCompositionQuestionCounts();
  const wrapper = document.createElement("div");
  wrapper.className = "sentence-composition-card sentence-composition-quiz-setup";
  const title = document.createElement("h2");
  title.id = "sentence-composition-title";
  title.textContent = "句子重組";
  const mode = document.createElement("p");
  ...
  description.className = "sentence-composition-meta";
  description.textContent = "每次測驗固定 5 題";
  const levels = document.createElement("div");
  levels.className = "sentence-composition-filters";
  levels.setAttribute("role", "group");
  levels.setAttribute("aria-label", "選擇句子重組測驗等級");
  [["N5", `N5：${counts.N5} 題`], ["N4", `N4：${counts.N4} 題`], ["mixed", `混合：${counts.all} 題`]].forEach(([level, label]) => {
    const button = createSentenceCompositionButton(label, `filter-button${selectedSentenceCompositionQuizLevel === level ? " is-active" : ""}`, () => {
      selectedSentenceCompositionQuizLevel = level;
      sentenceCompositionQuizStatusText = `已選擇${level === "mixed" ? "混合" : level}，可開始測驗設定。`;
      renderJapaneseSentenceCompositionQuizSetup();
    });
    button.setAttribute("aria-pressed", String(selectedSentenceCompositionQuizLevel === level));
  ...
  status.setAttribute("aria-live", "polite");
  status.textContent = sentenceCompositionQuizStatusText;
  const actions = document.createElement("div");
  actions.className = "sentence-composition-actions";
  const start = createSentenceCompositionButton("開始測驗", "answer-button", startSentenceCompositionQuizSetup);
  const back = createSentenceCompositionButton("返回文法選單", "secondary-button", returnToJapaneseGrammarMenu);
  actions.append(start, back);
  wrapper.append(title, mode, description, levels, status, actions);
  sentenceCompositionContent.replaceChildren(wrapper);
}

function startSentenceCompositionQuizSetup() {
  if (!selectedSentenceCompositionQuizLevel) {
    sentenceCompositionQuizStatusText = "請先選擇 N5、N4 或混合。";
    renderJapaneseSentenceCompositionQuizSetup();
    return;
  }
...
function openJapaneseSentenceCompositionPractice() {
  activeMode = "grammar";
  sentenceCompositionMode = "practice";
  resetJapaneseSentenceCompositionQuizSetupState();
  resetJapaneseSentenceCompositionPracticeState();
  renderJapaneseGrammarView("sentence-composition-practice");
  renderJapaneseSentenceCompositionPractice();
}

function openJapaneseSentenceCompositionQuizSetup() {
  activeMode = "grammar";
...
japaneseGrammarEntryButtons.forEach((button) => {
  button.addEventListener("click", (event) => {
    ...
    if (button.dataset.japaneseGrammarEntry === "sentence-composition-practice") {
      openJapaneseSentenceCompositionPractice();
      return;
    }
    if (button.dataset.japaneseGrammarEntry === "sentence-composition-quiz") {
...
if (startJapaneseJlptMockButton) {
  startJapaneseJlptMockButton.addEventListener("click", startJapaneseJlptMock);
}
...
function validateJapaneseJlptListeningCapability(provider) {
  ...
  if (!synthesis || typeof synthesis.getVoices !== "function" ||
    ...
    return Object.freeze({ ok: false, code: "speech-unavailable", message: "無法開始聽力測驗，請確認裝置語音功能後再試。" });
  let voices;
  ...
  const japaneseVoice = Array.isArray(voices)
    ...
    : null;
  if (!japaneseVoice)
    return Object.freeze({ ok: false, code: "japanese-voice-unavailable", message: "無法開始聽力測驗，請確認裝置語音功能後再試。" });
  // SpeechSynthesisVoice is a browser-owned host object. Freeze only our wrapper.
  return Object.freeze({ ok: true, japaneseVoice });
}
...
function buildJapaneseJlptListeningIsolatedSession(level, allCandidates, randomProvider = Math.random) {
  ...
  const selected = shuffleJapaneseJlptListening(levelCandidates, randomProvider).slice(0, JAPANESE_JLPT_LISTENING_SESSION_SIZE);
  const snapshot = deepFreezeJapaneseJlptValue(selected.map((candidate) => ({
    ...candidate, options: candidate.options.slice(),
  })));
  const extraPositions = shuffleJapaneseJlptListening([0, 1, 2, 3], randomProvider).slice(0, 2);
  const answerPositionInventory = [0, 1, 2, 3].flatMap((position) =>
    Array(extraPositions.includes(position) ? 3 : 2).fill(position));
  const answerPositions = shuffleJapaneseJlptListening(answerPositionInventory, randomProvider);
  const questions = snapshot.map((candidate, questionIndex) => {
...
function createJapaneseJlptListeningIsolatedController(provider) {
  ...
  let playedSourceIds = new Set(); let activeUtterance = null; let japaneseVoice = null; let disposed = false;
  const cancelActive = () => {
    generation += 1;
  ...
  return {
    start(level, candidates, randomProvider = Math.random) {
      ...
      japaneseVoice = capability.ok ? capability.japaneseVoice : null;
    ...
    reset() { cancelActive(); session = null; japaneseVoice = null; playedSourceIds = new Set(); questionIndex = 0; },
    dispose() { cancelActive(); session = null; japaneseVoice = null; playedSourceIds = new Set(); disposed = true; },
  };
}
...
function createJapaneseListeningModeSpeechController(provider, { maxPlaysPerItem = null } = {}) {
  ...
  return {
    requestPlayback(itemId, text, onStatus) {
      ...
      let utterance;
      try {
        if (!provider || !provider.speechSynthesis || typeof provider.speechSynthesis.speak !== "function" ||
    ...
    hasPlayback(itemId) { return maxPlaysPerItem !== 1 || !playedItemIds.has(itemId); },
    cancel,
    resetPlayback() { cancel(); playedItemIds = new Set(); },
  };
}

function getJapaneseListeningSpeechProvider() {
  return {
...
const listeningPracticeSpeechController = createJapaneseListeningModeSpeechController(getJapaneseListeningSpeechProvider());
const listeningQuizSpeechController = createJapaneseListeningModeSpeechController(getJapaneseListeningSpeechProvider(), { maxPlaysPerItem: 1 });

function normalizeJapaneseListeningView(view) {
  return ["menu", "practice", "quiz"].includes(view) ? view : "menu";
}

function cancelJapaneseListeningSpeech(mode = "all") {
  // Ownership is enforced by each controller; never call window.speechSynthesis.cancel() globally here.
  if (mode === "all" || mode === "practice") listeningPracticeSpeechController.cancel();
  if (mode === "all" || mode === "quiz") listeningQuizSpeechController.cancel();
}
...
function resetJapaneseListeningState({ resetPracticeIndex = false } = {}) {
  ...
  listeningQuizItems = [];
  listeningQuizSpeechController.resetPlayback();
...
function createListeningPlayButton(item, status, { onePlay = false } = {}) {
  const controller = onePlay ? listeningQuizSpeechController : listeningPracticeSpeechController;
  const button = document.createElement("button");
...
function renderJapaneseListeningPractice() {
  listeningQuizSpeechController.cancel();
  listeningQuizIndex = 0;
  listeningQuizCorrectCount = 0;
  listeningQuizAnswered = false;
  listeningQuizSelectedIndex = null;
...
function startJapaneseListeningQuiz() {
  listeningPracticeSpeechController.cancel();
  listeningQuizIndex = 0;
  listeningQuizCorrectCount = 0;
  listeningQuizAnswered = false;
  listeningQuizSelectedIndex = null;
